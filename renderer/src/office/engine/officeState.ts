...
/** Furniture IDs (or prefixes) that are considered plants for the breathing animation */
export const PLANT_FURNITURE_IDS = new Set([
  'PLANT',
  'PLANT_2',
  'LARGE_PLANT',
  'CACTUS',
...
/** Check whether a furniture type ID is a plant (exact match or prefix match against known plant IDs) */
export function isPlantFurniture(type: string): boolean {
  if (PLANT_FURNITURE_IDS.has(type)) return true;
  // Prefix match: e.g. "PLANT_2_FRONT_OFF" starts with "PLANT_2"
  for (const id of PLANT_FURNITURE_IDS) {
    if (type.startsWith(id)) return true;
  }
  return false;
}

/** Unique ID for the monitor NPC character */
export const MONITOR_NPC_ID = -999;

/** Simple hash code for strings (sum of char codes mod prime) */
export function hashCode(str: string): number {
  let hash = 0;
  for (let i = 0; i < str.length; i++) {
    hash = (hash + str.charCodeAt(i)) % 7919;
  }
  return hash;
}
...
export class OfficeState {
  ...
  /** When true, the renderer should skip drawing to avoid showing inconsistent state */
  isRebuilding = false;
  ...
  /** Create the monitor NPC if it doesn't already exist */
  createMonitorNpc(): void {
    if (this.characters.has(MONITOR_NPC_ID)) return;
    if (this.walkableTiles.length === 0) return;

    // Spawn inside the Monitor room (rightmost area, col >= totalCols - 8).
    // Fall back to the last walkable tile if the room isn't found.
    const totalCols = this.layout?.cols ?? 20;
    let spawn = this.walkableTiles[this.walkableTiles.length - 1];
    const monitorRoomTiles = this.walkableTiles.filter((t) => t.col >= totalCols - 8);
    if (monitorRoomTiles.length > 0) {
      // Pick the center-ish tile of the monitor room
      spawn = monitorRoomTiles[Math.floor(monitorRoomTiles.length / 2)];
    }
    const ch = createCharacter(MONITOR_NPC_ID, 3, null, null, 0);
    ch.x = spawn.col * TILE_SIZE + TILE_SIZE / 2;
    ch.y = spawn.row * TILE_SIZE + TILE_SIZE / 2;
    ch.tileCol = spawn.col;
    ch.tileRow = spawn.row;
    ...
    // No spawn effect for NPC
    ch.matrixEffect = null;
    ch.matrixEffectTimer = 0;
    ch.matrixEffectSeeds = [];
    this.characters.set(MONITOR_NPC_ID, ch);
  }
   ...
   *  Build the new array first, then swap the reference atomically to avoid a frame
   *  where `this.furniture` is empty (which would cause a visual flash). */
  private rebuildFurnitureInstances(): void {
    ...
    if (autoOnTiles.size === 0) {
      // Build new array first, then swap atomically
      const newFurniture = layoutToFurnitureInstances(this.layout.furniture);
    ...
    // Build new array first, then swap atomically
    const newFurniture = layoutToFurnitureInstances(modifiedFurniture);
    this.furniture = newFurniture;
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      this.withOwnSeatUnblocked(ch, () =>
        updateCharacter(ch, dt, this.walkableTiles, this.seats, this.tileMap, this.blockedTiles, this.furniture),
      );
