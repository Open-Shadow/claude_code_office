...
import { MonitorPanel, type SessionSummaryData } from './monitor/MonitorPanel.js';
...
function App() {
  ...
  const [openTerminal, setOpenTerminal] = useState<{ agentId: number; projectName: string } | null>(null);
  const [isMonitorVisible, setIsMonitorVisible] = useState(false);
  const [monitorSummaries, setMonitorSummaries] = useState<SessionSummaryData[]>([]);
  const [isHelpOpen, setIsHelpOpen] = useState(false);

  // Sync persisted locale to i18n
  useEffect(() => {
  ...
  const handleToggleMonitor = useCallback(() => {
    setIsMonitorVisible((prev) => {
      const next = !prev;
      vscode.invoke('toggle-monitor', { enable: next }).catch(console.error);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (data?.type === 'monitor-update') {
        setMonitorSummaries(data.summaries as SessionSummaryData[]);
      }
    };
  ...
  const handleDoubleClickAgent = useCallback((agentId: number) => {
    const os = getOfficeState();
    // Resolve sub-agent to parent
    const meta = os.subagentMeta.get(agentId);
    const resolvedId = meta ? meta.parentAgentId : agentId;
    const ch = os.characters.get(resolvedId);
    const projectName = ch?.folderName ?? `Agent ${resolvedId}`;
    setOpenTerminal({ agentId: resolvedId, projectName });
  }, []);
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative overflow-hidden">
      ...
      {!isDebugMode ? (
        <>
          <ZoomControls zoom={editor.zoom} onZoomChange={editor.handleZoomChange} onReset={handleResetView} />
      ...
      <BottomToolbar
        ...
        isMonitorOpen={isMonitorVisible}
        onToggleMonitor={handleToggleMonitor}
      ...
      {isMonitorVisible && (
        <MonitorPanel
          ...
          onClose={() => {
            ...
            vscode.invoke('toggle-monitor', { enable: false }).catch(console.error);
          }}
        />
      )}
