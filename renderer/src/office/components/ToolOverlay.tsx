...
function getActivityText(
  ...
  if (tools && tools.length > 0) {
    ...
    if (activeTool) {
      if (activeTool.permissionWait) return t('overlay.needsApproval');
...
export function ToolOverlay({
  ...
  // All character IDs (include monitor NPC for label rendering)
  const allIds = [...agents, ...subagentCharacters.map((s) => s.id)];
  if (officeState.characters.has(MONITOR_NPC_ID) && !allIds.includes(MONITOR_NPC_ID)) {
    allIds.push(MONITOR_NPC_ID);
  }
  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        if (isSub) {
          if (subHasPermission) {
            activityText = t('overlay.needsApproval');
          } else {
            const sub = subagentCharacters.find((s) => s.id === id);
            activityText = sub ? sub.label : t('overlay.subtask');
          }
        } else {
          activityText = getActivityText(id, agentTools, ch.isActive, t);
        }
        ...
        const teamRoleLabel = ch.isTeamLead ? t('overlay.lead') : ch.agentName || null;
          ...
          >
            ...
            {isTeamAgent && totalTokens > 0 && (
              <div
                ...
                title={t('overlay.contextUsed', { pct: Math.round(tokenRatio * 100), k: (totalTokens / 1000).toFixed(0) })}
              >
                <div
                  style={{
