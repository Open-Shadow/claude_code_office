...
import { hashCode, isPlantFurniture } from './officeState.js';
...
export function renderScene(
  ...
  for (const f of furniture) {
    ...
    let fy = offsetY + f.y * zoom;

    // Plant breathing animation — scale-based swaying anchored at bottom-center
    if (f.type && isPlantFurniture(f.type)) {
      const phase = hashCode(f.uid ?? f.type) * 0.1;
      const t = time * PLANT_BREATHE_SPEED * Math.PI * 2 + phase;
      const scaleX = 1 + Math.sin(t) * PLANT_BREATHE_AMPLITUDE;
      const scaleY = 1 + Math.sin(t + 1.0) * (PLANT_BREATHE_AMPLITUDE * 0.67);
      const spriteW = cached.width;
      const spriteH = cached.height;
      const centerX = fx + spriteW / 2;
      const bottomY = fy + spriteH;
      const mirroredF = f.mirrored;
      drawables.push({
        zY: f.zY,
        draw: (c) => {
          c.save();
          c.translate(centerX, bottomY);
          c.scale(mirroredF ? -scaleX : scaleX, scaleY);
          c.drawImage(cached, -spriteW / 2, -spriteH);
          c.restore();
        },
      });
      continue;
...
export function renderFrame(
  ...
  renderScene(ctx, allFurniture, characters, offsetX, offsetY, zoom, selectedId, hoveredId, time ?? 0);
