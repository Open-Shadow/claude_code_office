...
import { MONITOR_NPC_ID } from '../engine/officeState.js';
import type {
  DeleteButtonBounds,
  EditorRenderState,
  RotateButtonBounds,
...
export function OfficeCanvas({
  ...
  const lastClickRef = useRef<{ time: number; agentId: number | null }>({ time: 0, agentId: null });
  ...
  useEffect(() => {
    ...
    const stop = startGameLoop(canvas, {
      ...
      render: (ctx) => {
        // Skip rendering while layout is being rebuilt to avoid showing inconsistent state
        if (officeState.isRebuilding) return;
        // Canvas dimensions are in device pixels
        const w = canvas.width;
        const h = canvas.height;
        // Use furniture animation timer as accumulated time for plant breathing
        const time = officeState.furnitureAnimTimer;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      const pos = screenToWorld(e.clientX, e.clientY);
      if (!pos) return;
      const rawHitId = officeState.getCharacterAt(pos.worldX, pos.worldY);
      // Skip monitor NPC from interaction
      const hitId = rawHitId === MONITOR_NPC_ID ? null : rawHitId;
      const tile = screenToTile(e.clientX, e.clientY);
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (hitId !== null && hitId !== MONITOR_NPC_ID) {
        // Double-click detection: same agent clicked within 300ms
        const now = Date.now();
        const last = lastClickRef.current;
        if (last.agentId === hitId && now - last.time < 300) {
          // Double-click — open terminal
          lastClickRef.current = { time: 0, agentId: null };
          onDoubleClickAgent?.(hitId);
          return;
        }
        lastClickRef.current = { time: now, agentId: hitId };
      ...
      // No agent hit — reset double-click tracking so A→empty→A doesn't misfire
      lastClickRef.current = { time: 0, agentId: null };

      // No agent hit — check plugin furniture click
      const tile = screenToTile(e.clientX, e.clientY);
      if (tile) {
        const layout = officeState.getLayout();
        for (const f of layout.furniture) {
          if (!f.type.startsWith('PLUGIN_')) continue;
          const entry = getCatalogEntry(f.type);
          if (!entry) continue;
          if (
            tile.col >= f.col &&
            ...
            // Extract plugin ID from furniture metadata or find matching plugin
            // The plugin ID is stored in the manifest; we derive it from the furniture type
            vscode.postMessage({ type: 'open-plugin-panel', furnitureId: f.type });
            return;
          }
        }

        // Check whiteboard click → toggle monitor
        for (const f of layout.furniture) {
          if (!f.type.includes('WHITEBOARD')) continue;
          const entry = getCatalogEntry(f.type);
          if (!entry) continue;
          if (
            tile.col >= f.col &&
      ...
      // Check seat click while agent is selected
    ...
    [officeState, onClick, onDoubleClickAgent, onToggleMonitor, screenToWorld, screenToTile, isEditMode],
  );
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        officeState.cameraFollowId = null;
        if (e.shiftKey) {
          // Shift+scroll: map vertical scroll to horizontal panning, keep Y unchanged
